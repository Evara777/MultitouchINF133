...
public class MainDrawingScene extends AbstractScene
{
  ...
  // private String imagesPath = System.getProperty("user.dir")+File.separator +
  // "examples"+ File.separator +"advanced"+ File.separator + File.separator
  // +"drawing"+ File.separator + File.separator +"data"+ File.separator +
  // File.separator +"images" + File.separator ;
  private String imagesPath = "advanced" + AbstractMTApplication.separator
      + "drawing" + AbstractMTApplication.separator + "data"
      + AbstractMTApplication.separator + "images"
  ...
  public MainDrawingScene(AbstractMTApplication mtApplication, String name)
  {
    ...
    {
      System.err
          .println("Drawing example can only be run in OpenGL mode on a gfx card supporting the GL_EXT_framebuffer_object extension!");
      return;
    }
    this.registerGlobalInputProcessor(new CursorTracer(mtApplication, this));

    // Create window frame
    MTRoundRectangle frame = new MTRoundRectangle(pa, -50, -50, 0,
    ...
    // final MTSceneTexture sceneWindow = new MTSceneTexture(0,0, pa,
    // drawingScene);
    // We have to create a fullscreen fbo in order to save the image
    // uncompressed
    final MTSceneTexture sceneTexture = new MTSceneTexture(pa, 0, 0, pa.width,
    ...
    b.addGestureListener(TapProcessor.class, new IGestureEventListener()
    {
      public boolean processGestureEvent(MTGestureEvent ge)
      {
        ...
        if(te.isTapped())
        {
          // //As we are messing with opengl here, we make sure it happens in
    ...
    // final MTColorPicker colorWidget = new MTColorPicker(0,
    // pa.height-colPick.height, colPick, pa);
    ...
    MTPolygon p = new MTPolygon(pa, new Vertex[] {
  ...
  public boolean destroy()
  {
    ...
    if(destroyed)
    {
      drawingScene.destroy(); // Destroy the scene manually since it isnt
                              // destroyed in the MTSceneTexture atm!
    }
    return destroyed;
  }
}
